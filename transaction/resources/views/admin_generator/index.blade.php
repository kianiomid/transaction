...
@section('content')
    <section class="content-header">
        @if(\Illuminate\Support\Facades\App::getLocale() == 'fa')
            ...
            <h1 class="pull-right admin-list-h1">{{ $title }}</h1>
            <h1 class="pull-left admin-list-actions">
                @if(view()->exists($customView . '.list_actions'))
                    @include($customView . '.list_actions')
                @else
                    @include('admin_generator.list_actions')
                @endif
            </h1>
        @else
            <h1 class="pull-left admin-list-h1">{{ $title }}</h1>
                ...
                @if(sizeof($items) > 0)
                    <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                        ...
                        <div class="row" style="margin-top: 5px">
                            @include('admin_generator.list_batch_actions')
    ...
    @include("admin_generator.filters")
    <div class="clearfix"></div>
@endsection
...
@section('scripts')
    ...
    <script>
        ...
        $(".dataTable th.sortable").click(function () {
            window.location.href = "{{ route($modelName . '.index') }}?sort=" + $(this).attr('aria-sort') + "&sortOrder=" + $(this).attr('aria-sort-order');
        });
    </script>
@endsection